...
public:
    ...
    struct Policy {
        ...
        static_assert(!(kKBlockColumn % Shape::kColumn), "KBlock size must be divisible by warp shape.");
...
private:
    ...
    using AccumulatorIterations =
        MatrixShape<AccumulatorShape::kRow / InstructionShape::kM, AccumulatorShape::kColumn / InstructionShape::kN>;
    ...
    static int const kKBlockColumnIterations = kKBlockColumn / Shape::kColumn * (AccumulatorShape::kRow / Shape::kRow);
    static int const kResidualIndex = kResidualColumn / Shape::kColumn * (AccumulatorShape::kRow / Shape::kRow);
...
public:
    ...
    CUTLASS_HOST_DEVICE
    void load(Fragment &frag, ScaleBiasFragment &scale, ScaleBiasFragment &bias, OutputOp output_op) const {
        if (output_op.is_source_needed())  // beta must be zero
            assert(0);
...
template <
    ...
    typename OutputOp_>
class MmaTensorOpFragmentIterator<Shape_, AccumulatorShape_, KBlocksColumn_, ElementAccumulator_, Element_,
                                  cutlass::layout::RowMajor, InstructionShape_, OutputOp_> {
public:
    /// Shape of warp tile to load (concept: MatrixShape)
    using Shape = Shape_;
...
private:
    ...
    using AccessIterations =
        MatrixShape<MmaIterations::kRow * kIterationsPerInstruction, MmaIterations::kColumn / kIterationsPerAccess>;
