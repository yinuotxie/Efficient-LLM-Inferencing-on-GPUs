...
struct BlockInfo
{
    ...
    __device__ BlockInfo(const Params &params, const int bidb, const int bidh)
        : batch_idx(bidb),
          ...
          sum_seq_q(params.cu_seqlens_q[bidb]),
          sum_seq_k(params.cu_seqlens_k[bidb]),
          actual_seqlen_q(params.cu_seqlens_q[bidb + 1] - sum_seq_q),
          actual_seqlen_k(params.cu_seqlens_k[bidb + 1] - sum_seq_k),
